...
class _BrowserWorker(threading.Thread):
    ...
    def __init__(self) -> None:
        super().__init__(daemon=True, name="LocalFun-PaheBrowser")
    ...
    def submit(self, fn: Callable[[], T], timeout: float = 300.0) -> T:
        ...
        if self._ctx is None or self._page is None:
            raise RuntimeError(
                ...
                "windows from LocalFun, then use Settings -> Pass AnimePahe "
                "Cloudflare again."
            )
...
def _open_kwik_and_capture_m3u8(pg, kwik_url: str, *, referer: str) -> str:
    ...
    try:
        ...
        from localfun.services.kwik import extract_m3u8
...
def browser_download_episode(
    ...
    *,
    ...
    def _resolve() -> tuple[str, str, Path, Any, Any]:
        from localfun.services.animepahe import (
            PahePlayMeta,
            PaheSource,
    ...
    from localfun.services.animepahe import DownloadResult
    from localfun.services.hls_download import download_hls_curl
