...
from anidex.db.schema import ANIME_STATUSES, MANGA_STATUSES
from anidex.paths import app_data_dir, db_path, output_dir, tokens_path
from anidex.web.deps import repo_ctx
from anidex.web.jobs import JOBS
from anidex.web import security as websec
from anidex.web.security import redact_job_result, redact_path
...
def discover_anime() -> dict[str, Any]:
    ...
    from anidex.services.anime_discover import build_discover
    from anidex.services.mal_api import MalClient, MalApiError
...
def discover_manga() -> dict[str, Any]:
    from anidex.services.manga_discover import build_manga_discover
...
def upcoming_anime(
    ...
    from anidex.services.anime_countdown import build_upcoming
    from anidex.services.mal_api import MalClient, MalApiError
...
def upcoming_manga(
    ...
    from anidex.services.manga_countdown import build_manga_upcoming
...
def search_anime(q: str, limit: int = 20) -> list[dict[str, Any]]:
    ...
    from anidex.services.mal_api import MalClient
...
def search_manga(q: str, limit: int = 20) -> list[dict[str, Any]]:
    from anidex.services.mangadex import MangaDexClient
...
def manga_cover(mangadex_id: str, size: str = "256"):
    ...
    from anidex.services.mangadex import MangaDexClient, MangaDexError
...
def manga_chapters(
    ...
    from anidex.services import manga_offline as offline
    from anidex.services.mangadex import MangaDexClient, MangaDexError
...
def manga_chapter_read(
    ...
    from anidex.services.manga_offline import list_page_files
...
def cf_status() -> dict[str, Any]:
    try:
        from anidex.services.pahe_browser import cloudflare_cleared
...
def mal_sync(mode: str = "pull") -> dict[str, Any]:
    def work(job) -> dict[str, Any]:
        from anidex.services.mal_api import MalClient
        from anidex.services.sync import pull_mal_list, push_local_to_mal
...
async def import_xml(file: UploadFile = File(...)) -> dict[str, Any]:
    from anidex.services.sync import import_xml_into_repo
...
def export_xml():
    from fastapi.responses import FileResponse
    from anidex.services.sync import export_repo_to_xml

    out = app_data_dir() / "anidex_export.xml"
    with repo_ctx() as repo:
        export_repo_to_xml(repo, out)
    return FileResponse(out, filename="anidex_export.xml", media_type="application/xml")
